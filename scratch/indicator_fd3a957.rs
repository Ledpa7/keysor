use std::sync::OnceLock;
use std::thread;
use std::sync::atomic::{AtomicU32, Ordering};
use windows_sys::Win32::Foundation::{HWND, LPARAM, LRESULT, WPARAM, POINT, RECT};
use windows_sys::Win32::Graphics::Gdi::{
    BeginPaint, EndPaint, CreateSolidBrush, FillRect, DeleteObject, SelectObject, CreatePen, Ellipse,
    InvalidateRect, UpdateWindow, RoundRect, SetTextColor, SetBkMode, DrawTextW,
    CreateFontW
};
use windows_sys::Win32::UI::WindowsAndMessaging::{
    RegisterClassW, CreateWindowExW, DefWindowProcW, ShowWindow, SetWindowPos, MSG,
    WNDCLASSW, CS_HREDRAW, CS_VREDRAW, WS_POPUP, WS_EX_LAYERED, WS_EX_TRANSPARENT,
    WS_EX_TOPMOST, WS_EX_NOACTIVATE, SW_HIDE, SW_SHOWNA, LWA_COLORKEY, LWA_ALPHA, WM_PAINT,
    WM_DESTROY, GetMessageW, TranslateMessage, DispatchMessageW, HWND_TOPMOST,
    SWP_NOSIZE, SWP_NOACTIVATE, SetLayeredWindowAttributes, GetClientRect, GetSystemMetrics,
    WS_EX_APPWINDOW, WM_CLOSE, GetSystemMenu, AppendMenuW,
    MF_SEPARATOR, MF_STRING, WM_SYSCOMMAND, SW_MINIMIZE, WS_MINIMIZEBOX, WS_SYSMENU
};

pub static INDICATOR_HWND: OnceLock<HWND> = OnceLock::new();
pub static HUD_HWND: OnceLock<HWND> = OnceLock::new();
pub static MAIN_HWND: OnceLock<HWND> = OnceLock::new();
pub static HUD_HOVER: AtomicU32 = AtomicU32::new(0); // 0: none, 1: minimize, 2: close

const WM_MOUSELEAVE: u32 = 0x02A3;

fn encode_wide(s: &str) -> Vec<u16> {
    s.encode_utf16().chain(std::iter::once(0)).collect()
}

fn open_notepad_config() {
    let yaml_path = crate::config::get_config_path();
    std::process::Command::new("notepad.exe")
        .arg(yaml_path)
        .spawn()
        .ok();
}

fn open_explorer_config_folder() {
    let mut folder_path = crate::config::get_config_path();
    folder_path.pop();
    std::process::Command::new("explorer.exe")
        .arg(folder_path)
        .spawn()
        .ok();
}

fn get_startup_shortcut_path() -> std::path::PathBuf {
    let mut path = std::env::var("APPDATA")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|_| std::path::PathBuf::from("."));
    path.push("Microsoft");
    path.push("Windows");
    path.push("Start Menu");
    path.push("Programs");
    path.push("Startup");
    path.push("keysor.lnk");
    path
}

fn toggle_startup_shortcut() {
    let shortcut_path = get_startup_shortcut_path();
    if shortcut_path.exists() {
        std::fs::remove_file(&shortcut_path).ok();
        unsafe {
            windows_sys::Win32::UI::WindowsAndMessaging::MessageBoxW(
                0,
                encode_wide("?쒖옉 ?꾨줈洹몃옩?먯꽌 ?ㅼ냼??Keysor)媛 ?뺤긽 ?댁젣?섏뿀?듬땲??").as_ptr(),
                encode_wide("Keysor ?뚮┝").as_ptr(),
                0,
            );
        }
    } else {
        if let Ok(exe_path) = std::env::current_exe() {
            let exe_path_str = exe_path.to_string_lossy();
            let exe_dir = exe_path.parent().map(|p| p.to_string_lossy().to_string()).unwrap_or_default();
            let shortcut_path_str = shortcut_path.to_string_lossy();
            
            let script = format!(
                "$WshShell = New-Object -ComObject WScript.Shell; \
                 $Shortcut = $WshShell.CreateShortcut('{}'); \
                 $Shortcut.TargetPath = '{}'; \
                 $Shortcut.WorkingDirectory = '{}'; \
                 $Shortcut.Save()",
                shortcut_path_str, exe_path_str, exe_dir
            );
            
            #[cfg(target_os = "windows")]
            {
                use std::os::windows::process::CommandExt;
                std::process::Command::new("powershell")
                    .args(&["-Command", &script])
                    .creation_flags(0x08000000) // CREATE_NO_WINDOW
                    .status()
                    .ok();
            }
                
            unsafe {
                windows_sys::Win32::UI::WindowsAndMessaging::MessageBoxW(
                    0,
                    encode_wide("?쒖옉 ?꾨줈洹몃옩???ㅼ냼??Keysor)媛 ?깅줉?섏뿀?듬땲??\n遺?????먮룞 ?ㅽ뻾?⑸땲??").as_ptr(),
                    encode_wide("Keysor ?뚮┝").as_ptr(),
                    0,
                );
            }
        }
    }
}

fn draw_key_cap(
    hdc: windows_sys::Win32::Graphics::Gdi::HDC,
    font: windows_sys::Win32::Graphics::Gdi::HFONT,
    x: i32,
    y: i32,
    w: i32,
    h: i32,
    key_text: &str,
    desc_text: &str,
    category: u32, // 0: none, 1: Caps Lock, 2: WASD, 3: QERF, 4: Spacebar
) {
    unsafe {
        let bg_color = 0x202424; 
        let border_color = match category {
            1 => 0x2FFFAD, // Caps Lock (Bright Lime Green)
            2 => 0x40E000, // WASD Movement (Vibrant Grass Green)
            3 => 0xD0F8A0, // QERF Scroll/Wheel (Soft Mint Green)
            4 => 0x00AA80, // Spacebar Click (Deep Teal Green)
            _ => 0x3C4040, // Non-highlighted (Grey)
        };
        
        let brush = CreateSolidBrush(bg_color);
        let pen = CreatePen(0, 1, border_color);

        let old_brush = SelectObject(hdc, brush);
        let old_pen = SelectObject(hdc, pen);
        let old_font = SelectObject(hdc, font);

        RoundRect(hdc, x, y, x + w, y + h, 8, 8);

        SetTextColor(hdc, if category > 0 { border_color } else { 0x888888 });
        let key_w = encode_wide(key_text);
        let mut r_key = RECT { left: x, top: y + 4, right: x + w, bottom: y + 21 };
        DrawTextW(hdc, key_w.as_ptr(), key_w.len() as i32 - 1, &mut r_key, 1 | 32);

        SetTextColor(hdc, if category > 0 { 0xFFFFFF } else { 0x555555 });
        let desc_w = encode_wide(desc_text);
        let mut r_desc = RECT { left: x + 2, top: y + 23, right: x + w - 2, bottom: y + h - 4 };
        DrawTextW(hdc, desc_w.as_ptr(), desc_w.len() as i32 - 1, &mut r_desc, 1 | 32);

        SelectObject(hdc, old_font);
        SelectObject(hdc, old_brush);
        SelectObject(hdc, old_pen);
        
        DeleteObject(brush);
        DeleteObject(pen);
    }
}

unsafe extern "system" fn main_wnd_proc(
    hwnd: HWND,
    msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
) -> LRESULT {
    unsafe {
        match msg {
            WM_SYSCOMMAND => {
                let cmd_id = wparam & 0xFFFF;
                println!("[Debug] main_wnd_proc WM_SYSCOMMAND: cmd_id={}", cmd_id);
                match cmd_id {
                    1001 => {
                        open_notepad_config();
                        return 0;
                    }
                    1002 => {
                        open_explorer_config_folder();
                        return 0;
                    }
                    1003 => {
                        toggle_startup_shortcut();
                        return 0;
                    }
                    _ => {}
                }
                DefWindowProcW(hwnd, msg, wparam, lparam)
            }
            windows_sys::Win32::UI::WindowsAndMessaging::WM_SIZE => {
                let state = wparam;
                println!("[Debug] main_wnd_proc WM_SIZE: state={}", state);
                if let Some(&hud) = HUD_HWND.get() {
                    if state == 1 { // SIZE_MINIMIZED
                        ShowWindow(hud, SW_HIDE);
                    } else if state == 0 { // SIZE_RESTORED
                        ShowWindow(hud, SW_SHOWNA);
                    }
                }
                0
            }
            windows_sys::Win32::UI::WindowsAndMessaging::WM_ACTIVATE => {
                let state = wparam & 0xFFFF;
                println!("[Debug] main_wnd_proc WM_ACTIVATE: state={}", state);
                if state != 0 { // Activated
                    if let Some(&hud) = HUD_HWND.get() {
                        ShowWindow(hud, SW_SHOWNA);
                    }
                }
                DefWindowProcW(hwnd, msg, wparam, lparam)
            }
            WM_CLOSE | WM_DESTROY => {
                crate::hook::cleanup_hook();
                std::process::exit(0);
            }
            _ => {
                if msg == windows_sys::Win32::UI::WindowsAndMessaging::WM_SHOWWINDOW {
                    println!("[Debug] main_wnd_proc WM_SHOWWINDOW: wparam={}", wparam);
                }
                DefWindowProcW(hwnd, msg, wparam, lparam)
            }
        }
    }
}

unsafe extern "system" fn indicator_wnd_proc(
    hwnd: HWND,
    msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
) -> LRESULT {
    unsafe {
        match msg {
            WM_PAINT => {
                let mut ps = std::mem::zeroed();
                let hdc = BeginPaint(hwnd, &mut ps);

                let mut rect = RECT { left: 0, top: 0, right: 0, bottom: 0 };
                GetClientRect(hwnd, &mut rect);
                let bg_brush = CreateSolidBrush(0xFF00FF);
                FillRect(hdc, &rect, bg_brush);
                DeleteObject(bg_brush);

                let green_color = 0x81B910;
                let green_brush = CreateSolidBrush(green_color);
                let green_pen = CreatePen(0, 1, green_color);

                let old_brush = SelectObject(hdc, green_brush);
                let old_pen = SelectObject(hdc, green_pen);

                Ellipse(hdc, 4, 4, 28, 28);

                let magenta_brush = CreateSolidBrush(0xFF00FF);
                let magenta_pen = CreatePen(0, 1, 0xFF00FF);

                SelectObject(hdc, magenta_brush);
                SelectObject(hdc, magenta_pen);

                Ellipse(hdc, 10, 10, 22, 22);

                SelectObject(hdc, old_brush);
                DeleteObject(green_brush);
                DeleteObject(magenta_brush);

                SelectObject(hdc, old_pen);
                DeleteObject(green_pen);
                DeleteObject(magenta_pen);

                EndPaint(hwnd, &ps);
                0
            }
            WM_DESTROY => {
                0
            }
            _ => DefWindowProcW(hwnd, msg, wparam, lparam),
        }
    }
}

fn adjust_sensitivity(delta: f64) {
    if let Some(state_arc) = crate::hook::APP_STATE.get() {
        let mut state = state_arc.lock().unwrap();
        let mut new_speed = state.config.settings.base_speed + delta;
        if new_speed < 0.1 {
            new_speed = 0.1;
        } else if new_speed > 10.0 {
            new_speed = 10.0;
        }
        new_speed = (new_speed * 10.0).round() / 10.0;
        
        state.config.settings.base_speed = new_speed;
        crate::config::save_config(&state.config);
    }
}

fn toggle_pixel_mode() {
    if let Some(state_arc) = crate::hook::APP_STATE.get() {
        let mut state = state_arc.lock().unwrap();
        let cur = state.config.settings.pixel_mode.unwrap_or(false);
        state.config.settings.pixel_mode = Some(!cur);
        crate::config::save_config(&state.config);
    }
}

fn toggle_language() {
    if let Some(state_arc) = crate::hook::APP_STATE.get() {
        let mut state = state_arc.lock().unwrap();
        let cur = state.config.settings.lang_en.unwrap_or(false);
        state.config.settings.lang_en = Some(!cur);
        crate::config::save_config(&state.config);
    }
}

unsafe extern "system" fn hud_wnd_proc(
    hwnd: HWND,
    msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
) -> LRESULT {
    unsafe {
        match msg {
            WM_PAINT => {
                let mut ps = std::mem::zeroed();
                let hdc = BeginPaint(hwnd, &mut ps);

                let mut rect = RECT { left: 0, top: 0, right: 0, bottom: 0 };
                GetClientRect(hwnd, &mut rect);

                let bg_brush = CreateSolidBrush(0x121210);
                let border_pen = CreatePen(0, 2, 0x81B910);
                
                let old_brush = SelectObject(hdc, bg_brush);
                let old_pen = SelectObject(hdc, border_pen);
                
                RoundRect(hdc, 0, 0, rect.right, rect.bottom, 16, 16);
                
                SetBkMode(hdc, 1);
                
                let font_name = encode_wide("Segoe UI");
                let title_font = CreateFontW(22, 0, 0, 0, 700, 0, 0, 0, 1, 0, 0, 5, 0, font_name.as_ptr());
                let text_font = CreateFontW(13, 0, 0, 0, 400, 0, 0, 0, 1, 0, 0, 5, 0, font_name.as_ptr());
                let key_font = CreateFontW(12, 0, 0, 0, 600, 0, 0, 0, 1, 0, 0, 5, 0, font_name.as_ptr());
                let btn_font = CreateFontW(18, 0, 0, 0, 700, 0, 0, 0, 1, 0, 0, 5, 0, font_name.as_ptr());
                let number_font = CreateFontW(36, 0, 0, 0, 700, 0, 0, 0, 1, 0, 0, 5, 0, font_name.as_ptr());

                let pm_enabled = {
                    let state_arc = crate::hook::APP_STATE.get();
                    state_arc.map_or(false, |arc| arc.lock().unwrap().config.settings.pixel_mode.unwrap_or(false))
                };
                let lang_en = {
                    let state_arc = crate::hook::APP_STATE.get();
                    state_arc.map_or(false, |arc| arc.lock().unwrap().config.settings.lang_en.unwrap_or(false))
                };

                let q_desc = if lang_en { "Scroll ?" } else { "??ㅽ겕濡? };
                let w_desc = if lang_en { "Move ?? } else { "???대룞" };
                let e_desc = if lang_en { "Scroll ?? } else { "?ㅽ겕濡ㅲ뼳" };
                let r_desc = if lang_en { "Wheel ?? } else { "?졻뼯" };
                let caps_desc = if lang_en { "ON/OFF" } else { "???ㅽ봽" };
                let a_desc = if lang_en { "Move ?" } else { "? ?대룞" };
                let s_desc = if lang_en { "Move ?? } else { "???대룞" };
                let d_desc = if lang_en { "Move ?? } else { "???대룞" };
                let f_desc = if lang_en { "Wheel ?? } else { "?졻뼹" };
                let g_desc = if lang_en { "R-Click" } else { "?고겢" };
                let space_desc = if lang_en { "Click (1:L / 2:Db / Hold:Drag)" } else { "?대┃ (1:醫?/ 2:?붾툝 / ????쒕옒洹?" };
                
                let title_text = "??KEYSOR MOUSE EMULATION ACTIVE ??;
                let speed_sens_title = "SPEED SENS";
                
                let info1_text = if lang_en {
                    "??All alphabet typing is blocked during mouse mode (except Ctrl/Alt/Win shortcuts)."
                } else {
                    "??留덉슦??紐⑤뱶 以?紐⑤뱺 ?뚰뙆踰?????댄븨? 李⑤떒?⑸땲??(Ctrl, Alt, Win ?⑥텞???덉쇅 ?덉슜)."
                };
                let info2_text = if lang_en {
                    "??Press Caps Lock again to return to normal keyboard mode."
                } else {
                    "??Caps Lock????踰????꾨Ⅴ硫??쇰컲 ?ㅻ낫???곹깭濡?蹂듦??⑸땲??"
                };
                let info_close_text = if lang_en {
                    "Automatically minimizes when mouse mode (Caps Lock) is active."
                } else {
                    "留덉슦???쒖꽦??Caps Lock) ???먮룞?쇰줈 理쒖냼?붾맗?덈떎."
                };

                let old_font = SelectObject(hdc, title_font);
                SetTextColor(hdc, 0x81B910);
                let title = encode_wide(title_text);
                let mut r_title = RECT { left: 20, top: 15, right: rect.right - 20, bottom: 45 };
                DrawTextW(hdc, title.as_ptr(), title.len() as i32 - 1, &mut r_title, 1 | 32);

                // Draw Minimize and Close Buttons in top right corner
                let hover_val = HUD_HOVER.load(Ordering::SeqCst);

                // Draw Language Toggle Button
                let lang_bg_color = if hover_val == 6 { 0x3C4040 } else { 0x202424 };
                let lang_border_color = if hover_val == 6 { 0x81B910 } else { 0x3C4040 };
                let lang_text_color = if hover_val == 6 { 0x81B910 } else { 0x888888 };
                
                let lang_brush = CreateSolidBrush(lang_bg_color);
                let lang_pen = CreatePen(0, 1, lang_border_color);
                let old_brush_lang = SelectObject(hdc, lang_brush);
                let old_pen_lang = SelectObject(hdc, lang_pen);
                RoundRect(hdc, 630, 15, 680, 35, 6, 6);
                SelectObject(hdc, old_brush_lang);
                SelectObject(hdc, old_pen_lang);
                DeleteObject(lang_brush);
                DeleteObject(lang_pen);
                
                let lang_font = CreateFontW(12, 0, 0, 0, 700, 0, 0, 0, 1, 0, 0, 5, 0, font_name.as_ptr());
                let old_font_lang = SelectObject(hdc, lang_font);
                SetTextColor(hdc, lang_text_color);
                let lang_btn_text = encode_wide(if lang_en { "KO" } else { "EN" });
                let mut r_lang = RECT { left: 630, top: 15, right: 680, bottom: 35 };
                DrawTextW(hdc, lang_btn_text.as_ptr(), lang_btn_text.len() as i32 - 1, &mut r_lang, 37); // DT_CENTER | DT_VCENTER | DT_SINGLELINE
                SelectObject(hdc, old_font_lang);
                DeleteObject(lang_font);
                
                // Draw Minimize Button
                let min_bg_color = if hover_val == 1 { 0x3C4040 } else { 0x202424 };
                let min_border_color = if hover_val == 1 { 0x81B910 } else { 0x3C4040 };
                let min_text_color = if hover_val == 1 { 0x81B910 } else { 0x888888 };
                
                let min_brush = CreateSolidBrush(min_bg_color);
                let min_pen = CreatePen(0, 1, min_border_color);
                let old_brush_min = SelectObject(hdc, min_brush);
                let old_pen_min = SelectObject(hdc, min_pen);
                RoundRect(hdc, 690, 15, 715, 35, 6, 6);
                SelectObject(hdc, old_brush_min);
                SelectObject(hdc, old_pen_min);
                DeleteObject(min_brush);
                DeleteObject(min_pen);
                
                let min_font = CreateFontW(16, 0, 0, 0, 700, 0, 0, 0, 1, 0, 0, 5, 0, font_name.as_ptr());
                let old_font_min = SelectObject(hdc, min_font);
                SetTextColor(hdc, min_text_color);
                let min_text = encode_wide("-");
                let mut r_min = RECT { left: 690, top: 11, right: 715, bottom: 31 };
                DrawTextW(hdc, min_text.as_ptr(), min_text.len() as i32 - 1, &mut r_min, 1 | 32);
                SelectObject(hdc, old_font_min);
                DeleteObject(min_font);

                // Draw Close Button
                let close_bg_color = if hover_val == 2 { 0x1A1A40 } else { 0x202424 };
                let close_border_color = if hover_val == 2 { 0x0045FF } else { 0x3C4040 };
                let close_text_color = if hover_val == 2 { 0x0045FF } else { 0x888888 };
                
                let close_brush = CreateSolidBrush(close_bg_color);
                let close_pen = CreatePen(0, 1, close_border_color);
                let old_brush_close = SelectObject(hdc, close_brush);
                let old_pen_close = SelectObject(hdc, close_pen);
                RoundRect(hdc, 720, 15, 745, 35, 6, 6);
                SelectObject(hdc, old_brush_close);
                SelectObject(hdc, old_pen_close);
                DeleteObject(close_brush);
                DeleteObject(close_pen);
                
                let close_font = CreateFontW(14, 0, 0, 0, 700, 0, 0, 0, 1, 0, 0, 5, 0, font_name.as_ptr());
                let old_font_close = SelectObject(hdc, close_font);
                SetTextColor(hdc, close_text_color);
                let close_text = encode_wide("X");
                let mut r_close = RECT { left: 720, top: 16, right: 745, bottom: 34 };
                DrawTextW(hdc, close_text.as_ptr(), close_text.len() as i32 - 1, &mut r_close, 1 | 32);
                SelectObject(hdc, old_font_close);
                DeleteObject(close_font);
                
                // Row 1: Numbers (Y = 60)
                draw_key_cap(hdc, key_font, 30, 60, 42, 42, "~", "", 0);
                draw_key_cap(hdc, key_font, 78, 60, 42, 42, "1", "", 0);
                draw_key_cap(hdc, key_font, 126, 60, 42, 42, "2", "", 0);
                draw_key_cap(hdc, key_font, 174, 60, 42, 42, "3", "", 0);
                draw_key_cap(hdc, key_font, 222, 60, 42, 42, "4", "", 0);
                draw_key_cap(hdc, key_font, 270, 60, 42, 42, "5", "", 0);
                draw_key_cap(hdc, key_font, 318, 60, 42, 42, "6", "", 0);
                draw_key_cap(hdc, key_font, 366, 60, 42, 42, "7", "", 0);
                draw_key_cap(hdc, key_font, 414, 60, 42, 42, "8", "", 0);
                draw_key_cap(hdc, key_font, 462, 60, 42, 42, "9", "", 0);
                draw_key_cap(hdc, key_font, 510, 60, 42, 42, "0", "", 0);

                // Row 2: Q Row (Y = 108)
                draw_key_cap(hdc, key_font, 30, 108, 66, 42, "Tab", "", 0);
                draw_key_cap(hdc, key_font, 102, 108, 42, 42, "Q", q_desc, 3);
                draw_key_cap(hdc, key_font, 150, 108, 42, 42, "W", w_desc, 2);
                draw_key_cap(hdc, key_font, 198, 108, 42, 42, "E", e_desc, 3);
                draw_key_cap(hdc, key_font, 246, 108, 42, 42, "R", r_desc, 3);
                draw_key_cap(hdc, key_font, 294, 108, 42, 42, "T", "", 0);
                draw_key_cap(hdc, key_font, 342, 108, 42, 42, "Y", "", 0);
                draw_key_cap(hdc, key_font, 390, 108, 42, 42, "U", "", 0);
                draw_key_cap(hdc, key_font, 438, 108, 42, 42, "I", "", 0);
                draw_key_cap(hdc, key_font, 486, 108, 42, 42, "O", "", 0);
                draw_key_cap(hdc, key_font, 534, 108, 42, 42, "P", "", 0);

                // Row 3: A Row (Y = 156)
                draw_key_cap(hdc, key_font, 30, 156, 78, 42, "Caps", caps_desc, 1);
                draw_key_cap(hdc, key_font, 114, 156, 42, 42, "A", a_desc, 2);
                draw_key_cap(hdc, key_font, 162, 156, 42, 42, "S", s_desc, 2);
                draw_key_cap(hdc, key_font, 210, 156, 42, 42, "D", d_desc, 2);
                draw_key_cap(hdc, key_font, 258, 156, 42, 42, "F", f_desc, 3);
                draw_key_cap(hdc, key_font, 306, 156, 42, 42, "G", g_desc, 3);
                draw_key_cap(hdc, key_font, 354, 156, 42, 42, "H", "", 0);
                draw_key_cap(hdc, key_font, 402, 156, 42, 42, "J", "", 0);
                draw_key_cap(hdc, key_font, 450, 156, 42, 42, "K", "", 0);
                draw_key_cap(hdc, key_font, 498, 156, 42, 42, "L", "", 0);

                // Row 4: Z Row & Enter (Y = 204)
                draw_key_cap(hdc, key_font, 30, 204, 96, 42, "Shift", "", 0);
                draw_key_cap(hdc, key_font, 132, 204, 42, 42, "Z", "", 0);
                draw_key_cap(hdc, key_font, 180, 204, 42, 42, "X", "", 0);
                draw_key_cap(hdc, key_font, 228, 204, 42, 42, "C", "", 0);
                draw_key_cap(hdc, key_font, 276, 204, 42, 42, "V", "", 0);
                draw_key_cap(hdc, key_font, 324, 204, 42, 42, "B", "", 0);
                draw_key_cap(hdc, key_font, 372, 204, 42, 42, "N", "", 0);
                draw_key_cap(hdc, key_font, 420, 204, 42, 42, "M", "", 0);
                draw_key_cap(hdc, key_font, 468, 204, 108, 42, "Enter", "", 0);

                // Row 5: Modifier & Space (Y = 252)
                draw_key_cap(hdc, key_font, 30, 252, 54, 42, "Ctrl", "", 0);
                draw_key_cap(hdc, key_font, 90, 252, 54, 42, "Win", "", 0);
                draw_key_cap(hdc, key_font, 150, 252, 54, 42, "Alt", "", 0);
                draw_key_cap(hdc, key_font, 210, 252, 216, 42, "Spacebar", space_desc, 4);
                draw_key_cap(hdc, key_font, 432, 252, 54, 42, "Alt", "", 0);
                draw_key_cap(hdc, key_font, 488, 252, 54, 42, "Win", "", 0);
                draw_key_cap(hdc, key_font, 548, 252, 28, 42, "", "", 0);



                // Draw Speed Sensitivity Panel
                let box_pen = CreatePen(0, 1, 0x3C4040); // Grey border
                let old_box_pen = SelectObject(hdc, box_pen);
                let box_brush = CreateSolidBrush(0x161818); // Darker carbon background
                let old_box_brush = SelectObject(hdc, box_brush);
                RoundRect(hdc, 590, 110, 730, 295, 12, 12);
                SelectObject(hdc, old_box_brush);
                SelectObject(hdc, old_box_pen);
                DeleteObject(box_brush);
                DeleteObject(box_pen);

                SetTextColor(hdc, 0x81B910); // Green
                let sens_title = encode_wide(speed_sens_title);
                let mut r_sens_title = RECT { left: 590, top: 125, right: 730, bottom: 145 };
                DrawTextW(hdc, sens_title.as_ptr(), sens_title.len() as i32 - 1, &mut r_sens_title, 1 | 32);

                let sens_val = {
                    let state_arc = crate::hook::APP_STATE.get();
                    state_arc.map_or(1.0, |arc| arc.lock().unwrap().config.settings.base_speed)
                };
                let old_font_number = SelectObject(hdc, number_font);
                SetTextColor(hdc, 0xFFFFFF); // White
                let sens_val_text = encode_wide(&format!("{:.1}", sens_val));
                let mut r_sens_val = RECT { left: 590, top: 155, right: 730, bottom: 200 };
                DrawTextW(hdc, sens_val_text.as_ptr(), sens_val_text.len() as i32 - 1, &mut r_sens_val, 1 | 32);
                SelectObject(hdc, old_font_number);

                // Draw [-] Button
                let dec_bg_color = if hover_val == 3 { 0x3C4040 } else { 0x202424 };
                let dec_border_color = if hover_val == 3 { 0x81B910 } else { 0x3C4040 };
                let dec_text_color = if hover_val == 3 { 0x81B910 } else { 0x888888 };
                
                let dec_brush = CreateSolidBrush(dec_bg_color);
                let dec_pen = CreatePen(0, 1, dec_border_color);
                let old_brush_dec = SelectObject(hdc, dec_brush);
                let old_pen_dec = SelectObject(hdc, dec_pen);
                RoundRect(hdc, 610, 215, 650, 255, 8, 8);
                SelectObject(hdc, old_brush_dec);
                SelectObject(hdc, old_pen_dec);
                DeleteObject(dec_brush);
                DeleteObject(dec_pen);
                
                let old_font_dec = SelectObject(hdc, number_font);
                SetTextColor(hdc, dec_text_color);
                let dec_text = encode_wide("-");
                let mut r_dec = RECT { left: 610, top: 205, right: 650, bottom: 250 };
                DrawTextW(hdc, dec_text.as_ptr(), dec_text.len() as i32 - 1, &mut r_dec, 1 | 32);
                SelectObject(hdc, old_font_dec);

                // Draw [+] Button
                let inc_bg_color = if hover_val == 4 { 0x3C4040 } else { 0x202424 };
                let inc_border_color = if hover_val == 4 { 0x81B910 } else { 0x3C4040 };
                let inc_text_color = if hover_val == 4 { 0x81B910 } else { 0x888888 };
                
                let inc_brush = CreateSolidBrush(inc_bg_color);
                let inc_pen = CreatePen(0, 1, inc_border_color);
                let old_brush_inc = SelectObject(hdc, inc_brush);
                let old_pen_inc = SelectObject(hdc, inc_pen);
                RoundRect(hdc, 670, 215, 710, 255, 8, 8);
                SelectObject(hdc, old_brush_inc);
                SelectObject(hdc, old_pen_inc);
                DeleteObject(inc_brush);
                DeleteObject(inc_pen);
                
                let old_font_inc = SelectObject(hdc, btn_font);
                SetTextColor(hdc, inc_text_color);
                let inc_text = encode_wide("+");
                let mut r_inc = RECT { left: 670, top: 220, right: 710, bottom: 255 };
                DrawTextW(hdc, inc_text.as_ptr(), inc_text.len() as i32 - 1, &mut r_inc, 1 | 32);
                SelectObject(hdc, old_font_inc);

                // Draw Pixel Mode Toggle Button
                let pm_bg_color = if pm_enabled {
                    if hover_val == 5 { 0x92CA20 } else { 0x81B910 } // Lighter green if hovered
                } else {
                    if hover_val == 5 { 0x3C4040 } else { 0x202424 } // Lighter gray if hovered
                };
                let pm_border_color = if hover_val == 5 { 0x81B910 } else { 0x3C4040 };
                let pm_text_color = if pm_enabled || hover_val == 5 { 0xFFFFFF } else { 0x888888 };

                let pm_brush = CreateSolidBrush(pm_bg_color);
                let pm_pen = CreatePen(0, 1, pm_border_color);
                let old_brush_pm = SelectObject(hdc, pm_brush);
                let old_pen_pm = SelectObject(hdc, pm_pen);
                RoundRect(hdc, 610, 260, 710, 288, 6, 6);
                SelectObject(hdc, old_brush_pm);
                SelectObject(hdc, old_pen_pm);
                DeleteObject(pm_brush);
                DeleteObject(pm_pen);

                let old_font_pm = SelectObject(hdc, key_font);
                SetTextColor(hdc, pm_text_color);
                let pm_label_str = if lang_en {
                    if pm_enabled { "PIXEL: ON" } else { "PIXEL: OFF" }
                } else {
                    if pm_enabled { "?쎌? ?⑥쐞: ON" } else { "?쎌? ?⑥쐞: OFF" }
                };
                let pm_text = encode_wide(pm_label_str);
                let mut r_pm = RECT { left: 610, top: 260, right: 710, bottom: 288 };
                DrawTextW(hdc, pm_text.as_ptr(), pm_text.len() as i32 - 1, &mut r_pm, 37); // DT_CENTER | DT_VCENTER | DT_SINGLELINE
                SelectObject(hdc, old_font_pm);

                // 4. Draw Info Footer & Countdown text (Y = 310, 330)
                SelectObject(hdc, text_font);
                SetTextColor(hdc, 0x888888);
                let info1 = encode_wide(info1_text);
                let mut r_info1 = RECT { left: 30, top: 310, right: rect.right - 30, bottom: 328 };
                DrawTextW(hdc, info1.as_ptr(), info1.len() as i32 - 1, &mut r_info1, 0);

                // ?곗륫??媛?대뱶 臾멸뎄 異쒕젰
                SelectObject(hdc, text_font);
                let info_close = encode_wide(info_close_text);
                SetTextColor(hdc, 0x888888);
                let mut r_info_close = RECT { left: rect.right - 420, top: 331, right: rect.right - 30, bottom: 349 };
                DrawTextW(hdc, info_close.as_ptr(), info_close.len() as i32 - 1, &mut r_info_close, 2); // Right aligned (2 = DT_RIGHT)

                // 4. ?쇰컲 蹂듦? ?덈궡 ?띿뒪?몃뒗 醫뚯륫 ?섎떒?쇰줈 ?대룞
                SelectObject(hdc, text_font);
                let info2 = encode_wide(info2_text);
                SetTextColor(hdc, 0x888888); // ?쇰컲 ?띿뒪???뚯깋
                let mut r_info2 = RECT { left: 30, top: 331, right: rect.right - 350, bottom: 349 };
                DrawTextW(hdc, info2.as_ptr(), info2.len() as i32 - 1, &mut r_info2, 0);

                // Cleanup GDI
                SelectObject(hdc, old_font);
                DeleteObject(title_font);
                DeleteObject(text_font);
                DeleteObject(key_font);
                DeleteObject(btn_font);
                DeleteObject(number_font);
                
                SelectObject(hdc, old_brush);
                SelectObject(hdc, old_pen);
                DeleteObject(bg_brush);
                DeleteObject(border_pen);

                EndPaint(hwnd, &ps);
                0
            }

            windows_sys::Win32::UI::WindowsAndMessaging::WM_MOUSEMOVE => {
                let x = (lparam & 0xFFFF) as i16;
                let y = ((lparam >> 16) & 0xFFFF) as i16;
                
                let prev_hover = HUD_HOVER.load(Ordering::SeqCst);
                let mut new_hover = 0;
                
                if x >= 690 && x <= 715 && y >= 15 && y <= 35 {
                    new_hover = 1;
                } else if x >= 720 && x <= 745 && y >= 15 && y <= 35 {
                    new_hover = 2;
                } else if x >= 610 && x <= 650 && y >= 215 && y <= 255 {
                    new_hover = 3;
                } else if x >= 670 && x <= 710 && y >= 215 && y <= 255 {
                    new_hover = 4;
                } else if x >= 610 && x <= 710 && y >= 260 && y <= 288 {
                    new_hover = 5;
                } else if x >= 630 && x <= 680 && y >= 15 && y <= 35 {
                    new_hover = 6;
                }
                
                if new_hover != prev_hover {
                    HUD_HOVER.store(new_hover, Ordering::SeqCst);
                    InvalidateRect(hwnd, std::ptr::null(), 1);
                    
                    let mut tme = windows_sys::Win32::UI::Input::KeyboardAndMouse::TRACKMOUSEEVENT {
                        cbSize: std::mem::size_of::<windows_sys::Win32::UI::Input::KeyboardAndMouse::TRACKMOUSEEVENT>() as u32,
                        dwFlags: windows_sys::Win32::UI::Input::KeyboardAndMouse::TME_LEAVE,
                        hwndTrack: hwnd,
                        dwHoverTime: 0,
                    };
                    windows_sys::Win32::UI::Input::KeyboardAndMouse::TrackMouseEvent(&mut tme);
                }
                0
            }
            WM_MOUSELEAVE => {
                let prev_hover = HUD_HOVER.load(Ordering::SeqCst);
                if prev_hover != 0 {
                    HUD_HOVER.store(0, Ordering::SeqCst);
                    InvalidateRect(hwnd, std::ptr::null(), 1);
                }
                0
            }
            windows_sys::Win32::UI::WindowsAndMessaging::WM_LBUTTONDOWN => {
                let x = (lparam & 0xFFFF) as i16;
                let y = ((lparam >> 16) & 0xFFFF) as i16;
                
                if x >= 690 && x <= 715 && y >= 15 && y <= 35 {
                    if let Some(&main_hwnd) = MAIN_HWND.get() {
                        ShowWindow(main_hwnd, SW_MINIMIZE);
                    }
                } else if x >= 720 && x <= 745 && y >= 15 && y <= 35 {
                    if let Some(&main_hwnd) = MAIN_HWND.get() {
                        windows_sys::Win32::UI::WindowsAndMessaging::SendMessageW(
                            main_hwnd,
                            windows_sys::Win32::UI::WindowsAndMessaging::WM_CLOSE,
                            0,
                            0,
                        );
                    }
                } else if x >= 610 && x <= 650 && y >= 215 && y <= 255 {
                    adjust_sensitivity(-0.1);
                    InvalidateRect(hwnd, std::ptr::null(), 1);
                } else if x >= 670 && x <= 710 && y >= 215 && y <= 255 {
                    adjust_sensitivity(0.1);
                    InvalidateRect(hwnd, std::ptr::null(), 1);
                } else if x >= 610 && x <= 710 && y >= 260 && y <= 288 {
                    toggle_pixel_mode();
                    InvalidateRect(hwnd, std::ptr::null(), 1);
                } else if x >= 630 && x <= 680 && y >= 15 && y <= 35 {
                    toggle_language();
                    InvalidateRect(hwnd, std::ptr::null(), 1);
                } else {
                    // Make borderless window draggable!
                    windows_sys::Win32::UI::Input::KeyboardAndMouse::ReleaseCapture();
                    windows_sys::Win32::UI::WindowsAndMessaging::SendMessageW(
                        hwnd,
                        windows_sys::Win32::UI::WindowsAndMessaging::WM_NCLBUTTONDOWN,
                        2, // HTCAPTION
                        0,
                    );
                }
                0
            }
            WM_DESTROY => {
                0
            }
            _ => DefWindowProcW(hwnd, msg, wparam, lparam),
        }
    }
}

pub fn start_indicator() {
    thread::spawn(|| unsafe {
        let instance = windows_sys::Win32::System::LibraryLoader::GetModuleHandleW(std::ptr::null());
        
        // 1. Register Main Dummy class for Taskbar Icon
        let main_class_name = encode_wide("KeysorMainClass");
        let main_wnd_class = WNDCLASSW {
            style: CS_HREDRAW | CS_VREDRAW,
            lpfnWndProc: Some(main_wnd_proc),
            cbClsExtra: 0,
            cbWndExtra: 0,
            hInstance: instance,
            hIcon: 0,
            hCursor: 0,
            hbrBackground: 0,
            lpszMenuName: std::ptr::null(),
            lpszClassName: main_class_name.as_ptr(),
        };

        if RegisterClassW(&main_wnd_class) == 0 {
            eprintln!("[Error] Failed to register Main Dummy window class.");
            return;
        }

        // 2. Register Indicator class
        let class_name_wide = encode_wide("KeysorIndicatorClass");
        let wnd_class = WNDCLASSW {
            style: CS_HREDRAW | CS_VREDRAW,
            lpfnWndProc: Some(indicator_wnd_proc),
            cbClsExtra: 0,
            cbWndExtra: 0,
            hInstance: instance,
            hIcon: 0,
            hCursor: 0,
            hbrBackground: 0,
            lpszMenuName: std::ptr::null(),
            lpszClassName: class_name_wide.as_ptr(),
        };

        if RegisterClassW(&wnd_class) == 0 {
            eprintln!("[Error] Failed to register indicator window class.");
            return;
        }

        // 3. Register HUD class
        let hud_class_name = encode_wide("KeysorHUDClass");
        let hud_wnd_class = WNDCLASSW {
            style: CS_HREDRAW | CS_VREDRAW,
            lpfnWndProc: Some(hud_wnd_proc),
            cbClsExtra: 0,
            cbWndExtra: 0,
            hInstance: instance,
            hIcon: 0,
            hCursor: 0,
            hbrBackground: 0,
            lpszMenuName: std::ptr::null(),
            lpszClassName: hud_class_name.as_ptr(),
        };

        if RegisterClassW(&hud_wnd_class) == 0 {
            eprintln!("[Error] Failed to register HUD window class.");
            return;
        }

        // 4. Create Main Dummy Window for Taskbar Icon
        let main_title = encode_wide("Keysor (Keyboard Mouse)");
        let main_hwnd = CreateWindowExW(
            WS_EX_APPWINDOW,
            main_class_name.as_ptr(),
            main_title.as_ptr(),
            WS_POPUP | WS_MINIMIZEBOX | WS_SYSMENU,
            -200,
            -200,
            0,
            0,
            0,
            0,
            instance,
            std::ptr::null(),
        );

        if main_hwnd != 0 {
            MAIN_HWND.set(main_hwnd).ok();
            // ?쒖뒪??硫붾돱瑜??살뼱??而ㅼ뒪? ??ぉ 異붽?
            let sys_menu = GetSystemMenu(main_hwnd, 0);
            if sys_menu != 0 {
                AppendMenuW(sys_menu, MF_SEPARATOR, 0, std::ptr::null());
                AppendMenuW(sys_menu, MF_STRING, 1001, encode_wide("?숋툘 Keysor ?ㅼ젙 ?닿린 (keysor.yaml)").as_ptr());
                AppendMenuW(sys_menu, MF_STRING, 1002, encode_wide("?뱛 ?ㅼ젙 ?대뜑 ?닿린 (.keysor)").as_ptr());
                AppendMenuW(sys_menu, MF_STRING, 1003, encode_wide("?? ?쒖옉 ?꾨줈洹몃옩 ?깅줉/?댁젣 ?좉?").as_ptr());
            }
            ShowWindow(main_hwnd, SW_SHOWNA);
        }

        // 5. Create Indicator Window
        let hwnd = CreateWindowExW(
            WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST | WS_EX_NOACTIVATE,
            class_name_wide.as_ptr(),
            std::ptr::null(),
            WS_POPUP,
            0,
            0,
            32, // width
            32, // height
            0,
            0,
            instance,
            std::ptr::null(),
        );

        if hwnd == 0 {
            eprintln!("[Error] Failed to create indicator window.");
            return;
        }

        SetLayeredWindowAttributes(hwnd, 0xFF00FF, 0, LWA_COLORKEY);
        INDICATOR_HWND.set(hwnd).ok();

        // 6. Create HUD Window
        let screen_width = GetSystemMetrics(0); // SM_CXSCREEN
        let screen_height = GetSystemMetrics(1); // SM_CYSCREEN
        
        let hud_w = 760;
        let hud_h = 380;
        let hud_x = (screen_width - hud_w) / 2;
        let hud_y = (screen_height - hud_h) / 2;

        let hud_hwnd = CreateWindowExW(
            WS_EX_LAYERED | WS_EX_TOPMOST | WS_EX_NOACTIVATE,
            hud_class_name.as_ptr(),
            std::ptr::null(),
            WS_POPUP,
            hud_x,
            hud_y,
            hud_w,
            hud_h,
            main_hwnd, // Owner window
            0,
            instance,
            std::ptr::null(),
        );

        if hud_hwnd == 0 {
            eprintln!("[Error] Failed to create HUD window.");
            return;
        }

        SetLayeredWindowAttributes(hud_hwnd, 0, 230, LWA_ALPHA); 
        HUD_HWND.set(hud_hwnd).ok();

        // ??대㉧ ?쒓굅, 理쒖큹 ?앹뾽 ?곸떆 ?몄텧
        ShowWindow(hud_hwnd, SW_SHOWNA);
        InvalidateRect(hud_hwnd, std::ptr::null(), 1);
        UpdateWindow(hud_hwnd);

        println!("[Info] Indicator & HUD windows created successfully.");

        // Run message loop
        let mut msg: MSG = std::mem::zeroed();
        while GetMessageW(&mut msg, 0, 0, 0) > 0 {
            TranslateMessage(&msg);
            DispatchMessageW(&msg);
        }
    });
}

pub fn show_indicator() {
    println!("[Debug] show_indicator() called");
    if let Some(&main_hwnd) = MAIN_HWND.get() {
        unsafe {
            windows_sys::Win32::UI::WindowsAndMessaging::ShowWindow(main_hwnd, SW_MINIMIZE);
        }
    }
    if let Some(&hwnd) = INDICATOR_HWND.get() {
        unsafe {
            let mut pt = POINT { x: 0, y: 0 };
            windows_sys::Win32::UI::WindowsAndMessaging::GetCursorPos(&mut pt);
            SetWindowPos(
                hwnd,
                HWND_TOPMOST,
                pt.x - 16,
                pt.y - 16,
                0,
                0,
                SWP_NOSIZE | SWP_NOACTIVATE,
            );
            ShowWindow(hwnd, SW_SHOWNA);
            InvalidateRect(hwnd, std::ptr::null(), 1);
            UpdateWindow(hwnd);
        }
    }
}

pub fn hide_indicator() {
    println!("[Debug] hide_indicator() called");
    if let Some(&hwnd) = INDICATOR_HWND.get() {
        unsafe {
            ShowWindow(hwnd, SW_HIDE);
        }
    }
}

pub fn update_indicator_position() {
    if let Some(&hwnd) = INDICATOR_HWND.get() {
        unsafe {
            let mut pt = POINT { x: 0, y: 0 };
            windows_sys::Win32::UI::WindowsAndMessaging::GetCursorPos(&mut pt);
            SetWindowPos(
                hwnd,
                HWND_TOPMOST,
                pt.x - 16,
                pt.y - 16,
                0,
                0,
                SWP_NOSIZE | SWP_NOACTIVATE,
            );
            InvalidateRect(hwnd, std::ptr::null(), 1);
            UpdateWindow(hwnd);
        }
    }
}
